MOVE FILE TO: d:\Huace_Work\AI_Control\AI_Tune\deployment\model_deployment.py
"""
模型部署工具：将 PyTorch 权重转换为多种部署格式
支持：ONNX、TorchScript、量化权重等
"""
...
class ModelDeployer:
    ...
    def export_to_onnx(self, output_path=None):
        ...
        try:
            ...
            print(f"  使用 ONNX Runtime 推理: pip install onnxruntime")
            print(f"  参考代码见: deployment_reference.py")
            return output_path
    ...
    def export_to_torchscript(self, output_path=None):
        """
        导出为 TorchScript 格式（PyTorch 原生轻量级格式）
        
        TorchScript 优点：
        - PyTorch 原生，推理无需 Python 环境
        - 支持 C++ 部署
        - 较好的性能
        ...
        try:
            ...
            print(f"  使用方法: torch.jit.load('model.pt')")
            return output_path
            
        except Exception as e:
            print(f"✗ TorchScript 导出失败: {e}")
            print(f"  提示：某些模块可能不支持 jit.script，尝试使用 jit.trace")
            return None
    ...
    def export_to_quantized(self, output_path=None, quantization_type='int8'):
        ...
        Args:
            output_path: 输出文件路径，默认自动生成
            quantization_type: 量化类型 ('int8' 或 'dynamic')
        ...
        try:
            ...
            print(f"  量化后: {quantized_size_mb:.2f} MB")
...
def main():
    ...
    if choice == '1':
        deployer.export_to_onnx(os.path.join(output_dir, "model.onnx"))
    elif choice == '2':
        deployer.export_to_torchscript(os.path.join(output_dir, "model.pt"))
    elif choice == '3':
        deployer.export_to_quantized(
            os.path.join(output_dir, "model_quantized.pth"),
            quantization_type='dynamic'
        )
    else:
        deployer.export_all_formats(output_dir)
    
    print(f"\n✓ 导出完成，文件位置: {output_dir}")
