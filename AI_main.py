...
def _select_model_and_config(model_dir, models, allow_default_latest=False):
    while True:
        try:
            ...
            ts = model_name.replace("adaptive_net_", "").replace(".pth", "")
            config_path = os.path.join(model_dir, f"config_{ts}.pt")
...
def main():
    ...
    models = sorted([f for f in os.listdir(model_dir) if f.startswith("adaptive_net_") and f.endswith(".pth")], reverse=True)
    ...
    if models:
        ...
        while True:
            resp = input("\n请选择: [N]ew Train (重新训练), [S]elect Test (选择并测试), [R]etrain (重新训练并保存新版): ").lower()
            if resp in ['n', 's', 'r']:
                choice = resp
                break
    ...
    if choice == 's' and models:
        try:
            ...
            run_comparison(selected_model, selected_config, path_type=selected_path)
            return
        except:
            print("输入无效，默认重新训练。")
            choice = 'n'
    ...
    if choice in ['n', 'r']:
        ...
        if m_path and c_path:
            print("\n训练完成，正在进入对比测试...")
            selected_path = _select_test_trajectory()
            run_comparison(m_path, c_path, path_type=selected_path)
        else:
            print("训练未成功完成。")
